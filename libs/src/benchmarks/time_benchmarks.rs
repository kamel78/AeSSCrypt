...
use crate::{additionl_ae::{ascon_ae::Ascon128, gift_cofb::GiftCofb, sparkle_ae::Schwaemm256}, aes_ciphers::CipherName, aess::core::AeSSCipherCore, ccm::{aes_ccm::AesCcm, aes_ccm_256::AesCcm256}, gcm::{aes_gcm::AesGcm, aes_gcm_256::AesGcm256}, ocb::{aes_ocb::AesOcb, aes_ocb_256::AesOcb256}, galois_arithmetic::GF128};
...
pub fn bench_aes_aess_latency() {
    println!("\n=== AeSS-128 Latency Benchmark (10KB) ===\n");
    let key = 0x0123456789abcdef0123456789abcdef;
    ...
    let mut aess = AeSSCipherCore::new(&plaintext,size, true,&mut out1, 4,CipherName::AES128);
    aess.set_key_scheme(&[GF128::from(key),GF128::from(key)], &GF128::from(iv));
    // Warmup
    for _ in 0..10 {    let _ = aess.encrypt();}    
    // Measure encryption latency
    let iterations = 1000;
    let mut latencies = Vec::with_capacity(iterations);    
    for _ in 0..iterations {    let start = Instant::now();
...
pub fn bench_aess_256_latency() {
    println!("\n=== AESS-256 Latency Benchmark (10KB) ===\n");
    let key1 = 0x0123456789abcdef0123456789abcdef;
    let key2 = 0xfedcba9876543210fedcba9876543210;
    let iv = 0x0123456789abcdef0123456789abcdef;    
    let size = 10 * 1024; // 10KB
    ...
    let mut aess = AeSSCipherCore::new(&plaintext,size, true,&mut out1, 4,CipherName::AES128);
    aess.set_key_scheme(&[GF128::from(key1),GF128::from(key2)], &GF128::from(iv));
    // Warmup
    for _ in 0..10 {    let _ = aess.encrypt();}    
    // Measure encryption latency
    let iterations = 1000;
...
pub fn bench_aess_throughput() {
    println!("\n=== AESS-128 Throughput Benchmark (1GB) ===\n");
    let key = 0x0123456789abcdef0123456789abcdef;
    let iv = 0x0123456789abcdef0123456789abcdef;    
...
pub fn bench_aes_aess_256_throughput() {
    println!("\n=== AESS-256 Throughput Benchmark (1GB) ===\n");
    let key = 0x0123456789abcdef0123456789abcdef;
    let iv = 0x0123456789abcdef0123456789abcdef;    
...
pub fn bench_sparkle_throughput() {
    ...
    let sparkle = Schwaemm256::new(key);
